...
interface GrpcServerHandle extends AdapterServerHandle {
	_internal: {
		...
		isStreaming: boolean;
		schema?: LoadedSchema;
		onRequest?: (message: Message) => Promise<Message | null>;
	};
}
...
/**
 * gRPC Unary adapter options
 */
export interface GrpcUnaryAdapterOptions {
...
export class GrpcUnaryAdapter extends BaseSyncAdapter implements SyncAdapter {
	...
	/** Adapter options */
	private adapterOptions: GrpcUnaryAdapterOptions;

	/** Loaded gRPC schema */
	private schema?: LoadedSchema;

	constructor(options: GrpcUnaryAdapterOptions = {}) {
		super();
	...
	private createServiceImplementation(
		...
		isProxy: boolean,
		targetAddress?: { host: string; port: number },
		onRequestCallback?: (message: Message) => Promise<Message | null>,
	...
	async createClient(
		...
		// Get the service client constructor - prefer adapter options, then config options
		const serviceName =
			this.adapterOptions.serviceName ||
			((config.options as Record<string, unknown> | undefined)?.serviceName as
				| string
				| undefined);
...
/**
 * gRPC Stream adapter options
 */
export interface GrpcStreamAdapterOptions {
	/** Path to .proto file(s) */
	schema?: string | string[];
	/** Service name to use for client calls */
	serviceName?: string;
...
export class GrpcStreamAdapter extends BaseAsyncAdapter implements AsyncAdapter {
	...
	/** Adapter options */
	private adapterOptions: GrpcStreamAdapterOptions;
	...
	/**
	 * Get request handler for a server and method
	 */
	...
	private createBidiStreamHandler(
		...
		return (call) => {
			...
			if (isProxy && targetAddress && this.schema) {
				proxyCall = this.createProxyStream(
					targetAddress,
					methodName,
					call,
					grpcMetadata,
	...
	private createClientStreamHandler(
		...
		return (call, callback) => {
			...
			call.on("end", async () => {
				const handler = this.getRequestHandler(serverId, methodName);
				if (handler) {
					try {
	...
	async createClient(
		...
		// Prefer adapter options, then config options
		const serviceName =
			this.adapterOptions.serviceName ||
			((config.options as Record<string, unknown> | undefined)?.serviceName as
				| string
				| undefined);
		const methodName =
			this.adapterOptions.methodName ||
			((config.options as Record<string, unknown> | undefined)?.methodName as
				| string
				| undefined);
