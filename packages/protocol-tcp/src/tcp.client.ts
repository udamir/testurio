...
type EventHandlers<T extends HandlerType> = T extends 'data' ? DataHandler :
	T extends 'message' ? MessageHandler :
	T extends 'error' ? ErrorHandler :
	T extends 'end' ? EndHandler : never
...
export class TcpClient {
  ...
  on<T extends HandlerType>(event: T, handler: EventHandlers<T>) {
    switch (event) {
      case 'data': this.onData = handler as DataHandler; break
      case 'message': this.onMessage = handler as MessageHandler; break
      case 'error': this.onError = handler as ErrorHandler; break
      case 'end': this.onEnd = handler as EndHandler; break
    }
  }
  ...
  connect(host: string, port: number, cfg: TcpClientConfig) {
    ...
    const sock = this.config.tls 
      ? tls.connect({ host, port, servername: this.config.serverName, rejectUnauthorized: !this.config.insecureSkipVerify, timeout }) 
