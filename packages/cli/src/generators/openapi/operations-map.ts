...
function extractResponses(
  ...
  return Object.entries(responses).map(([code, response]) => {
    ...
    const schemaRef = schema
      ? (isArray && schema.items ? resolveSchemaName(schema.items) : resolveSchemaName(schema))
      : undefined;
 ...
 */
function findOrvalResponseName(
  operationId: string,
...
function findOrvalResponseItemName(
  operationId: string,
  exportedNames: Set<string>,
): string | undefined {
...
export function buildOperationsMap(
  ...
  for (const op of operations) {
    ...
    if (op.headerParams.length > 0) {
      if (hasOrvalHeaders) {
        ...
        headerSchemaName = `${op.operationId}HeadersSchema`;
        const fields = op.headerParams.map((p) => {
          const optional = !p.required ? '.optional()' : '';
          return `  ${safeKey(p.name)}: z.string()${optional},`;
        });
        headerSchemaEntries.push(
          `export const ${headerSchemaName} = z.object({\n${fields.join('\n')}\n});`,
        );
      }
    }
    ...
    const ifaceRequestParts: string[] = [
      `method: '${op.method}'`,
      `path: '${op.path}'`,
    ];
  ...
  return { headerSchemas, operationsMap, serviceInterface, typeHelpers };
}
