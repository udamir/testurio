...
import type { GrpcSource, OpenApiSource } from "./config/schema.js";
...
export function createCli(): Command {
	...
	program.name("testurio").description("Testurio CLI - Generate type-safe schemas for testing").version("0.4.0");
	...
	program
		...
		.option("-o, --output <path>", "Output file or directory (default: {input}.schema.ts)")
...
export function buildSourcesFromInputs(inputs: string[], output?: string): Array<OpenApiSource | GrpcSource> {
	...
	return resolvedFiles.map((input) => {
		...
		if (output) {
			if (resolvedFiles.length > 1) {
				...
				resolvedOutput = path.join(output, `${base}.schema.ts`);
			} else {
...
export function resolveOutputPath(input: string | string[]): string {
	...
	return path.join(dir, `${base}.schema.ts`);
}
...
async function generateFromSource(
	...
	logger.info(
		`Generating from ${resolvedSource.type}: ${typeof resolvedSource.input === "string" ? resolvedSource.input : resolvedSource.input.join(", ")}`
	);
