...
import { type Config, configSchema, SUPPORTED_EXTENSIONS } from "./schema.js";
...
export async function loadConfig(configPath?: string): Promise<{ config: Config; filepath: string }> {
	...
	if (!result || result.isEmpty) {
		throw new Error("No config file found. Run `testurio init` to create one, or use `--config` to specify a path.");
	}
...
export function expandDirectorySources(rawConfig: unknown, configDir: string): unknown {
	...
	for (const source of generate.sources) {
		...
		const supported = entries.filter((e) => SUPPORTED_EXTENSIONS.includes(path.extname(e).toLowerCase())).sort();
		...
		for (const file of supported) {
			...
			// Inherit output as directory: append {basename}.schema.ts
			if (s.output) {
				const base = path.basename(file, path.extname(file));
				expanded.output = path.join(String(s.output), `${base}.schema.ts`);
			}
