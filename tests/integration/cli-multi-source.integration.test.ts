...
import { buildSourcesFromInputs, configSchema, expandDirectorySources } from "@testurio/cli";
import type { GrpcSource, OpenApiSource } from "@testurio/cli/config/schema";
...
describe("Multi-source generation", () => {
	...
	describe("directory input — config mode", () => {
		it("expands directory source in config and auto-detects types", () => {
			...
			if (parsed.success) {
				const generate = parsed.data.generate;
				expect(generate).toBeDefined();
				const sources = generate?.sources ?? [];
				expect(sources.length).toBeGreaterThanOrEqual(2);
