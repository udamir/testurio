...
/**
 * Create a hook with isMatch function for message type matching
 */
function createHook(isMatch: (msg: Message) => boolean): Hook<Message> {
	return {
		id: "test-hook",
		componentName: "test",
		phase: "test",
...
describe("Hook Matching", () => {
	describe("message type matching", () => {
		it("should match exact message type", () => {
			...
			component.registerHook(createHook((msg) => msg.type === "Order"));
		...
		it("should match with function matcher", () => {
			const component = new TestComponent("test");
			component.registerHook(createHook((msg) => msg.type.startsWith("Order")));
	...
	describe("payload matching", () => {
		it("should match by traceId", () => {
			...
			component.registerHook(
				createHook((msg) => msg.type === "Order" && msg.traceId === "trace-123")
			);
		...
		it("should match by function", () => {
			...
			component.registerHook(
				createHook(
					(msg) => msg.type === "Order" && (msg.payload as { amount: number }).amount > 100
				)
			);
