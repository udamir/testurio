...
describe("CLI Config Schema", () => {
	...
	describe("resolveOutputPath", () => {
		...
		it("preserves directory path", () => {
			expect(resolveOutputPath("./api/petstore.yaml")).toBe(path.join("api", "petstore.schema.ts"));
		});

		it("preserves nested directory path", () => {
			expect(resolveOutputPath("./specs/v2/api.json")).toBe(path.join("specs", "v2", "api.schema.ts"));
		});
		...
		it("uses first element for array input", () => {
			expect(resolveOutputPath(["./proto/a.proto", "./proto/b.proto"])).toBe(path.join("proto", "a.schema.ts"));
		});
	});
