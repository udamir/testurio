...
describe("HookRegistry", () => {
	...
	describe("findMatchingHooks", () => {
		it("should find hooks matching message", () => {
			const hook: Hook = {
				id: "hook-1",
				componentName: "client",
				phase: "test",
			...
			// Each component owns its own registry, so no componentName needed
			const matches = registry.findMatchingHooks(message);

			expect(matches).toHaveLength(1);
			expect(matches[0].hook).toBe(hook);
		});

		it("should not find hooks for wrong message type", () => {
			const hook: Hook = {
				id: "hook-1",
				componentName: "client",
				phase: "test",
			...
			// Different message type should not match
			const message: Message = { type: "Payment", payload: {} };
			const matches = registry.findMatchingHooks(message);
		...
		it("should sort matches by score", () => {
			const hooks: Hook[] = [
				...
				},
				{
					...
					phase: "test",
					messageTypes: "Order",
					matcher: { type: "traceId", value: "trace-123" },
					handlers: [],
					persistent: false,
				},
			];
			...
			// traceId hook should come first (higher score)
			expect(matches[0].hook.id).toBe("hook-2");
			expect(matches[1].hook.id).toBe("hook-1");
		});
	});
