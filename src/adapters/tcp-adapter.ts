 ...
 */
export class TcpAdapter extends BaseProtocolAdapter {
	 ...
	 */
	async startServer(config: ServerConfig): Promise<TcpServerHandle> {
	...
	private handleClientData(handle: TcpClientHandle, data: Buffer): void {
		...
		while (handle._internal.buffer.length > 0) {
			...
			handle._internal.buffer = handle._internal.buffer.slice(delimiterIndex + 1);
	 ...
	 */
	private async deliverMessageToClient(handle: TcpClientHandle, incomingMessage: Message): Promise<void> {
